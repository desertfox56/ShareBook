...
const BookReadingPage = () => {
  ...
  const chapters = [
    {
      title: 'Глава 1',
      content: pdfText || 'Loading text from PDF...',
    },
    {
      title: 'Глава 2',
  ...
  // Функция для рендеринга содержимого главы, продумать как разбить по страницам содержание пдф файла
  const renderChapterContent = (chapter) => (
    <Typography>
      <Paragraph style={{ fontFamily: "'Baskervville', serif", fontWeight: 'normal', color:'#121111', fontSize:'16px' }}>
      <PDFTextExtractor pdfUrl="/для студентов 2024г. ПИ Методические рекомендации ДП .pdf" setText={setPdfText} />
      </Paragraph>
    </Typography>
  );
